...
```bash
python {SKILL_DIR}/survey_download.py download --id 问卷ID --output_dir "输出目录"
```
...
```bash
python {SKILL_DIR}/survey_download.py download --id 确定的ID --output_dir "目录"
```
